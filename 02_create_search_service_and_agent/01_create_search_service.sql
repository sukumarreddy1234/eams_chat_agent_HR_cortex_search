...
-----------------------------------------------------------------------------*/
CREATE
OR REPLACE CORTEX SEARCH SERVICE HR_POLICIES_DB.DEV.POLICY_SEARCH ON chunk --search column
ATTRIBUTES language,
relative_path --attributes are the columns that you’ll be able to filter search results on
WAREHOUSE = CORTEX_DEMO_WH TARGET_LAG = '1 hour' AS (
    SELECT
        chunk,
        RELATIVE_PATH,
        FILE_URL,
...
FROM
    CONTEXT_DOCS;
HR_POLICIES_DB.DEV.POLICY_SEARCHHR_POLICIES_DB.DEV.POLICY_SEARCH